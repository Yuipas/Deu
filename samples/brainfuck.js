...
class Brainfuck {
   ...
   toString() {
      ...
      this.pointers.forEach((v, i) =>
         content += (p === i ? ("*" + (v.toString()) + "*") : v.toString()) + (i < l ? ", " : ""));
      return "[" + content + "]";
   }
   ...
   parseToken() {
      ...
      if(char === "<") {
         ...
         this.pointer = Math.min(this.pointer + 1, this.memorySize);
      }
      ...
      if(char === ".") {
         console.log("[BF]: " + ASCII[this.pointers[this.pointer]]);
      }
   }
...
(function() {
   let cmd =
      ">>,[<++++>-],<.>[<<+>+++>-]<--.<---[>+<-]>..+++.>.,+[>+++++<-]>.[-]<<.+++.<,[>-<--]>-.++>,[<->-]<.[-]<<<<+.-";
   let interpreter = new Brainfuck(cmd, 10);

   // ASCII.forEach((v, i) => console.log(i + ": \"" + v + "\""));

   interpreter.run(); // prints "Hello World!"
   console.log("[STATE]: ", parser.toString());
})();
